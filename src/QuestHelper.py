...
from Scripts.omgarturo.src.fm_core.core_items import RARE_SERPENT_EGG_STATIC_ID
from Scripts.omgarturo.src.fm_core.core_items import SERPENT_NEST_STATIC_ID
from Scripts.omgarturo.src.fm_core.core_items import SNAKE_CHARMER_FLUTE_STATIC_ID
from Scripts.omgarturo.src.fm_core.core_items import CARRONADE_GRAPHIC_ID
from Scripts.omgarturo.src.fm_core.core_items import CANNON_GRAPHIC_IDS
from Scripts.omgarturo.src.fm_core.core_items import RAMROD_STATIC_ID
from Scripts.omgarturo.src.fm_core.core_mobiles import SILVER_SERPENT_MOBILE_ID
from Scripts.omgarturo.src.fm_core.core_mobiles import GIANT_SERPENT_MOBILE_ID
from Scripts.omgarturo.src.fm_core.core_items import PAINTS_AND_A_BRUSH_STATIC_ID
from Scripts.omgarturo.src.fm_core.core_items import BOW_GRAPHIC_ID
from Scripts.omgarturo.src.fm_core.core_items import HUNTING_PERMIT_GRAPHIC_ID
from Scripts.omgarturo.src.fm_core.core_items import DRYDOCK_SHIP_GRAPHIC_ID
...
def huntsman(stop, interval):
    ...
    GAME_ANIMAL_CORPSES = [
        ...
        "a kraken corpse",
        "a lion corpse",
        "a myrmidex drone corpse",
        "a myrmidex corpse",
        "a najasaurus corpse",
        "a platinum drake corpse",
        "a polar bear corpse",
        "a raptor corpse",
        "a saber-toothed tiger corpse",
        "a saurosaurus corpse",
        "a scorpion corpse",
        "a sea serpent corpse",
        "a triceratops corpse",
        "a turkey corpse",
        "a tyrannosaurus rex corpse",
        "a walrus corpse",
        "a tiger corpse"
    ]
    ...
    while not stop.WaitOne(interval) and not Player.IsGhost:  
        ...
        for permit in permits:
            if permit is not None and len(permit.Properties) < 4:
                ...
                corpses = [item for item in items if item.Name.lower() in GAME_ANIMAL_CORPSES and item.Serial not in corpseScannerCache]
