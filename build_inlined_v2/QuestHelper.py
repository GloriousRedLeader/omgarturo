...
def collector_quest(stop, interval):
    ...
    while not stop.WaitOne(interval) and (not Player.IsGhost):
        ...
        MONSTER_NAMES = ['a betrayer', 'a bogling', 'a bog thing', 'a juggernaut', 'a juka mage', 'a juka warrior', 'a lich', 'a meer mage', 'a meer warrior', 'a mongbat', 'a mummy', 'a plague beast', 'a sand vortex', 'a stone gargoyle', 'a swamp dragon', 'a wisp']
...
def huntsman(stop, interval):
    GAME_ANIMALS = ['an alligator', 'an allosaurus', 'an anchisaur', 'a bake kitsune', 'a boar', 'a bull', 'a cougar', 'a desert ostard', 'a dimetrosaur', 'an eagle', 'a high plains boura', 'a frenzie ostard', 'a gaman', 'a giant toad', 'a gorilla', 'a grey wolf', 'a grizzly bear', 'a hiryu', 'a kraken', 'a lion', 'a myrmidex drone', 'a myrmidex larvae', 'a najasaurus', 'a platinum drake', 'a polar bear', 'a raptor', 'saber-toothed tiger', 'a saurosaurus', 'a scorpion', 'a sea serpent', 'a triceratops', 'a turkey', 'a tyrannosaurus rex', 'a walrus', 'a wild tiger', 'a tiger']
    corpseScannerCache = []
    permitTurninCache = []
    while not stop.WaitOne(interval) and (not Player.IsGhost):
        ...
        Misc.Pause(1000)
BUTTONS = [[1, RARE_SERPENT_EGG_STATIC_ID, 'Medusa Egg Helper', 'Uses snake charming flute to lure snakes to egg nests.\nPicks up eggs from ground.', medusa_helper], [2, PAINTS_AND_A_BRUSH_STATIC_ID, 'Collector Quest', 'Will talk to NPCs and take photos of creatures when you\nget to that part.', collector_quest], [3, DRYDOCK_SHIP_GRAPHIC_ID, 'Fire Cannon!', 'Fires nearest cannon on a loop. Hopefully while you are on\na ship. Just stand near it. BOOM BOOM BOOM.', fire_nearest_cannon_loop], [4, HUNTING_PERMIT_GRAPHIC_ID, 'Huntsman Stuff', 'Alerts when relevant animals appear on screen.\nUses permit on corpses.\nTurns in permits and gets new permits.', huntsman]]
OUR_GUMP_ID = 3135176914
...
def render_gump(buttonid):
    ...
    for button in BUTTONS:
        ...
        Gumps.AddButton(atlasGump, 25, y, 1210, 1209, button[0], 1, 1)
...
STOP_EVENT_NAME = 'Local\\RazorEnhanced.OMGArthur.Stop'
