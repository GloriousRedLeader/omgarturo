...
if 'removetrap' in order:
    ...
    down = 3
    left = 4
    messages = {'success': 'You successfully disarm the trap!', 'fail': 'You fail to disarm the trap and reset it', 'wait': 'You must wait a few moments to use another skill'}
    run_data = {'coords': [0, 0], 'used_coords': [[0, 0]], 'size': 3, 'good_steps': [], 'incoming_dir': left, 'last_dir': up, 'successive_fails': 0, 'time_started': datetime.now()}
    fails = 0
    runs = 1
    avg_reset_time = 0
    last_puzzle_time = 0
def remTrap():
    ...
    def getNextDir(last_dir, incoming_dir, cur_coords, used_coords, size, cycle=0):
        if cycle > 3:
            Misc.SendMessage('Recursion too deep! Aborting recalculation...', 128)
            return last_dir
    ...
    while Player.GetRealSkillValue('Remove Trap') < Player.GetSkillCap('Remove Trap'):
        ...
        if midpoint_count == 7:
            ...
            run_data['size'] = 4
        if run_data['size'] == 0:
            Misc.SendMessage('Puzzle size indeterminate, stopping script', 272)
            break
        Misc.SendMessage('Puzzle Size: ' + str(run_data['size']), 96)
        Misc.SendMessage('Executing known steps: ' + str(map(lambda x: dirStr(x), run_data['good_steps'])), 128)
        Misc.SendMessage('Used coords: ' + str(run_data['used_coords']), 128)
        ...
        while not (Journal.Search(messages['success']) or Journal.Search(messages['fail'])):
            ...
            Misc.SendMessage('Trying ' + dirStr(run_data['next_dir']) + ', previous try ' + dirStr(run_data['last_dir']) + ', came from ' + dirStr(run_data['incoming_dir']), 73)
        ...
        if Journal.Search(messages['success']):
            ...
            Misc.SendMessage('Solve time: ' + str(last_puzzle_time), 57)
            reset(run_data)
        elif Journal.Search(messages['fail']):
            Misc.SendMessage('Failed', 57)
            run_data['successive_fails'] = run_data['successive_fails'] + 1
            fails = fails + 1
            if run_data['successive_fails'] > 3:
                Misc.SendMessage('Too many successive fails, resetting...', 128)
                if Gumps.HasGump():
                    Gumps.CloseGump(box_gump)
                reset(run_data)
        else:
            Misc.SendMessage('Timed out', 57)
    Misc.SendMessage('Training complete!', 144)
...
def armsLore():
    ...
    Misc.SendMessage('All done with ' + str(Player.GetRealSkillValue(skillName)) + '!.', 96)
def itemId():
    skillName = 'Item ID'
    initialSkill = Player.GetRealSkillValue(skillName)
    while Player.GetSkillCap(skillName) > Player.GetSkillValue(skillName):
...
def lockPicking():
    ...
    while Player.GetSkillCap(skillName) > Player.GetSkillValue(skillName):
        ...
        if useOwnPicks == True:
            ...
            if lockPickCount < 1:
                Misc.SendMessage('Ran out of lockpicks.  Moving on to next skill', 96)
                break
...
def camping():
    ...
    while Player.GetSkillCap(skillName) > Player.GetSkillValue(skillName):
        ...
        if kindlingCount < 1:
            Misc.SendMessage('Ran out of kindling.  Moving on to next skill', 96)
            break
