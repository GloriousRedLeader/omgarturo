...
def do_route(path, range=6, autoLootBufferMs=0, pathFindingTimeoutSeconds=3.0, tileOffset=0):
    sectorId = 0
    for coord in path:
        sectorId = sectorId + 1
        serialsToExclude = []
        while not Player.IsGhost:
            ...
            if len(eligible) > 0:
                ...
                break
def run_rail_loop(path, attackRange=5, pathFindingTimeoutSeconds=5.0, autoLootBufferMs=2000, tileOffset=0):
    rails_stats('start')
    while not Player.IsGhost:
...
run_rail_loop(path=new_haven_noob_dungeon, attackRange=5, pathFindingTimeoutSeconds=5.0, autoLootBufferMs=2000, tileOffset=0)
